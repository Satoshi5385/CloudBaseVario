"""Standalone Tk GUI for simulating CloudBaseVario vario sounds."""
...
except Exception as exc:  # Keep the GUI importable so it can explain the fix.
    np = None
    sd = None
    ...
    def start(self) -> None:
        if AUDIO_IMPORT_ERROR is not None or sd is None or np is None:
            raise RuntimeError(
                ...
                "tools/requirements-vario-simulator.txt.\n"
...
class VarioSoundSimulatorApp:
    ...
    def _build_parameter_editor(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            header,
            text="parameters.json audio parameters",
            style="Panel.TLabel",
    ...
    def _save_configuration_as(self) -> None:
        ...
        if not self.editor_valid:
            return
        initial_name = self.current_path.name if self.current_path else "parameters.json"
